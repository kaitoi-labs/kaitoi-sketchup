  ...
  module Dialogs
    ...
    module Dialog
      ...
      def attach_callbacks(dialog)
        ...
        dialog.add_action_callback('save_settings') do |_ctx, json|
          guard('save_settings') do
            ...
            incoming.delete('mcp_token') if incoming['mcp_token'].to_s.strip.empty?
      ...
      def redact(cfg)
        ...
        mcp = out['mcp_token'].to_s
        out['mcp_token']      = ''
        out['mcp_token_set']  = !mcp.empty?
        out['mcp_token_hint'] = mcp.empty? ? '' : "#{mcp[0, 6]}…#{mcp[-4, 4]}"
        out
      end
